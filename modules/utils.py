...
class Visualizer:
    ...
    def plot_rolling_offset_interactive(roll: float, run: float, set_val: float):
        ...
        # 2. Reference Lines (Triangle)
        # Side (Roll)
        fig.add_trace(go.Scatter3d(
            x=[0, roll], y=[0, 0], z=[0, 0],
            mode='lines',
            line=dict(color='green', width=4, dash='dash'),
            name='Versprung Seite (Roll)'
        ))
        
        # Length (Run) - projected on ground
        fig.add_trace(go.Scatter3d(
        ...
        fig.add_trace(go.Scatter3d(
            ...
            name='Versprung Höhe (Set)'
        ))

        # Connect base to make it look like a box/projection?
        # Maybe just the path along axes: (0,0,0) -> (roll,0,0) -> (roll,run,0) -> (roll,run,set)
        ...
        fig.add_trace(go.Scatter3d(
            ...
            z=[0, 0, set_val/2],
            mode='text',
            text=[f"Roll: {roll:.0f}", f"Run: {run:.0f}", f"Set: {set_val:.0f}"],
        ...
        fig.update_layout(
            scene=dict(
                xaxis=dict(title='Seite (Roll)', range=[-max_dim*0.1, max_dim*1.2]),
                yaxis=dict(title='Länge (Run)', range=[-max_dim*0.1, max_dim*1.2]),
                zaxis=dict(title='Höhe (Set)', range=[-max_dim*0.1, max_dim*1.2]),
                aspectmode='data' # Important for real proportions
            ),
