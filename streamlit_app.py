"""
PipeCraft Enterprise Edition (V35.1 - Uncompressed Gold Master)
---------------------------------------------------------------
COMPLETE SOURCE CODE. NO ABBREVIATIONS.

Features restored and integrated:
1.  V23.3 Detailed Calculation (Physics-based Welding/Cutting costs).
2.  V23.3 Stutzen-Table (Angles, Depths, Circumference).
3.  V34.0 Smart Cut with Multi-DN Support (Reducers).
4.  Inventory Management System (ERP).
5.  Full 3D Visualization.
...
try:
    ...
    PDF_AVAILABLE = False
    logger.warning("FPDF Library not found. PDF export will be unavailable.")

# Streamlit Page Config
st.set_page_config(
    page_title="PipeCraft V35.1 Enterprise",
    page_icon="🏗️",
    layout="wide",
...
try:
    ...
    st.error(f"FATAL ERROR: Data Integrity Violation. {e}")
    st.stop()
...
def get_row_by_dn(dn: int) -> pd.Series:
    """Safe retrieval of pipe data row."""
    try:
...
# UI Helper Functions (State Safe)
...
class SelectedFitting:
    """Represents an item in the cutting list."""
    type_name: str
    count: int
...
class FittingManager:
    ...
    def get_deduction(type_name: str, dn_target: int, pn_suffix: str = "_16", custom_angle: float = 45.0) -> float:
        ...
        if type_name == "Bogen 90° (BA3)":
            ...
            # Length of reducer is usually determined by the larger end
...
class PhysicsEngine:
    ...
    def calculate_pipe_weight(dn_idx: int, ws: float, length_mm: float, is_zme: bool = False) -> float:
        """Calculates pipe weight including optional ZME."""
        try:
...
class CostEngine:
    """
    Implements the granular cost logic from Version 23.3.
    Detailed breakdown of gas, wire, electrodes, and labor.
    """
    ...
    def calculate_welding_detailed(
        ...
        t_fit = zoll * 2.5 # Fitting time constant per inch
        ...
        # 2. Material Calculation (Physics-based)
        da = df_pipe[df_pipe['DN'] == dn].iloc[0]['D_Aussen']
        
        # Approximate V-seam volume
        ...
        # Filler material weight (Steel density) with 50% overhead
        ...
        if "CEL" in process:
            ...
            # Gas cost (Only during active welding, not fitting)
            weld_time_only_min = (t_weld * factor) * anz
            gas_cost = weld_time_only_min * 15 * p_gas # 15 L/min
...
if 'store' not in st.session_state:
    st.session_state.store = {
        ...
        # Prices
        'p_lohn': 60.0, 'p_stahl': 2.5, 'p_dia': 45.0, 'p_cel': 0.40, 
        'p_draht': 15.0, 'p_gas': 0.05, 'p_wks': 25.0, 'p_kebu1': 15.0, 
        'p_kebu2': 12.0, 'p_primer': 12.0, 'p_machine': 15.0
    }
...
with tab_werk:
    ...
    if "Säge" in tool_mode:
        ...
        if "Reduzierung" in f_type:
            c_red1, c_red2 = col_f2.columns(2)
            f_dn = c_red1.selectbox("DN (Groß)", df_pipe['DN'], index=df_pipe['DN'].tolist().index(selected_dn_global), key="red_large")
            f_dn_small = c_red2.selectbox("DN (Klein)", df_pipe['DN'], index=0, key="red_small")
        else:
        ...
        if st.button("Hinzufügen (+)", type="secondary"):
            ...
            if "Reduzierung" in f_type: name += f" -> DN {f_dn_small}"
        ...
        st.markdown("---")
        if final_cut < 0:
            st.error("Fehler: Bauteile sind länger als das Isomaß!")
        else:
            ...
            kg = PhysicsEngine.calculate_pipe_weight(dn_idx, WS_STD_MAP.get(selected_dn_global, 4.0), final_cut, c_zme)
...
# TAB 3: ROHRBUCH (RESTORED V23.3 FORM)
with tab_proj:
    ...
    with st.form("rb"):
        ...
        c6, c7 = st.columns(2)
        charge = c6.text_input("Charge")
        schweisser = c7.text_input("Schweißer")
        
        if st.form_submit_button("Speichern"):
            DatabaseRepository.add("rohrbuch", (iso, naht, datum.strftime("%d.%m.%Y"), f"DN {dn_s}", "Rohr", len_s, charge, schweisser))
...
# TAB 4: KALKULATION (RESTORED DETAIL)
with tab_info:
    ...
    if "Fügen" in task:
        ...
        cost = (dur/60) * (get_val('p_lohn') + get_val('p_machine'))
