"""
PipeCraft V34.0 (Stable Enterprise Edition - Reducer Logic)
-----------------------------------------------------------
Version: 34.0.0
Date: 2023-12-21
...
Changelog:
- FEAT: Specific UI Logic for Reducers (Large DN vs. Small DN).
- FEAT: Correct deduction mapping for concentric reducers based on Large OD.
- CORE: Preserved all V33.1 features (3D, Inventory, Physics).
"""
...
<style>
    ...
    .result-card-blue { background-color: #eff6ff; padding: 20px; border-radius: 10px; border-left: 6px solid #3b82f6; margin-bottom: 15px; color: #1e3a8a; }
    .result-card-green { background: linear-gradient(to right, #f0fdf4, #ffffff); padding: 25px; border-radius: 12px; border-left: 8px solid #22c55e; margin-bottom: 15px; text-align: center; font-size: 1.6rem; font-weight: 800; color: #14532d; }
    .detail-box { background-color: #f1f5f9; border: 1px solid #cbd5e1; padding: 15px; border-radius: 8px; text-align: center; font-size: 0.9rem; color: #334155; }
    .weight-box { background-color: #fff1f2; border: 1px solid #fecdd3; color: #be123c; padding: 15px; border-radius: 8px; text-align: center; font-weight: bold; margin-top: 10px; }
...
try:
    ...
    st.error(f"FATAL ERROR: Inconsistent Data. {e}")
    st.stop()
...
class FittingManager:
    ...
    def get_deduction(type_name: str, dn_target: int, pn_suffix: str = "_16", custom_angle: float = 45.0) -> float:
        """
        Berechnet das Z-Maß.
        dn_target: Bei Reduzierung ist das immer der GROSSE Durchmesser (Maßgebend für Länge).
        """
        ...
        if type_name == "Bogen 90° (BA3)":
            ...
            # Bei konzentrischer Reduzierung bestimmt der große DN die Länge (DIN/EN)
...
class CostEngine:
    ...
    def calculate_cutting_detailed(
        dn: int, ws: float, disc_type: str, anz: int, is_zma: bool, is_iso: bool, factor: float,
        p_lohn: float, p_stahl_disc: float, p_dia_disc: float
    ) -> Tuple[float, float, str]:
        
        zoll = dn / 25.0
        if "230" in disc_type: cap = 14000
        elif "180" in disc_type: cap = 7000
        else: cap = 3500
...
with tab_werk:
    ...
    if "Säge" in tool_mode:
        ...
        # Formteil-Auswahl mit Multi-DN Support
        col_f1, col_f2, col_f3, col_f4 = st.columns([2, 1, 1, 1])
        f_type = col_f1.selectbox("Typ", ["Bogen 90° (BA3)", "Bogen (Zuschnitt)", "Flansch (Vorschweiß)", "T-Stück", "Reduzierung (konz.)"])
        
        # Logik für Reduzierungen: Frage nach Groß und Klein
        f_dn = selected_dn_global # Default
        f_dn_small = None
        
        if "Reduzierung" in f_type:
            # Bei Reduzierung zeigen wir zwei Felder an, damit der User weiß, was er tut
        ...
        if st.button("Hinzufügen (+)", type="secondary"):
            ...
            if "Reduzierung" in f_type:
                name = f"Reduzierung {f_dn}x{f_dn_small}"
            if "Zuschnitt" in f_type:
                name += f" [{f_angle}°]"
        ...
        with c_c:
            if et_type == "2D":
                h = st.number_input("H", value=300)
                l = st.number_input("L", value=400)
